...
void CrearBuffer( const int w, const int h ){
	...
	zbuff = (int*)malloc( w * h * sizeof(int) ); // creamos un tamaño al buffer
...
void GuardarBufferZ( const char Name[] ){
	...
	int Len = wbuff*hbuff;
	while(Len--){
		*ptr = (0x101010101*((*ptr)>>16));
		ptr++;
	}

	SaveBMP( Name, (unsigned char*)zbuff, wbuff, hbuff, 32 );
}
