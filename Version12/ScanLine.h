...
void SetScanLine( int *src ){
	...
	// if (
	// 	(SLA[y0].x < x0 && SLA[y0].x < x1) &&
	// 	(SLB[y0].x > x1 && SLB[y0].x > x0) 
	// ) return;
...
void ScanLineFill( int color ){
	...
	// Linea( SLA[RC_SCAN[0]].x, 0, SLA[RC_SCAN[0]].x, (SS_ALTO-1) ,0xffff0000 );
	// Linea( SLB[RC_SCAN[1]].x, 0, SLB[RC_SCAN[1]].x, (SS_ALTO-1) ,0xffff0000 );
